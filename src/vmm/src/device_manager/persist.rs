...
impl<'a> Persist<'a> for MMIODeviceManager {
    ...
    fn restore(
        ...
        info!("PASS_debug Restoring MMIO devices...");
        #[cfg(target_arch = "aarch64")]
        {
        ...
        info!("PASS_debug dev_manager.register_mmio_virtio definitation finished...");
        if let Some(balloon_state) = &state.balloon_device {
            ...
            info!("PASS_debug balloon_state.device_id {} finished...",balloon_state.device_id);
            restore_helper(
                device.clone(),
                device,
        ...
        info!("PASS_debug block_state starting...");
        for block_state in &state.block_devices {
            ...
            info!("PASS_debug block_state.device_id {} finished...", block_state.device_id);
        }
        info!("PASS_debug block_state yes finished!");
        ...
        if let Some(vsock_state) = &state.vsock_device {
            ...
            info!("PASS_debug vsock_state.device_id {} finished...", vsock_state.device_id);
            restore_helper(
                device.clone(),
                device,
        ...
        if let Some(entropy_state) = &state.entropy_device {
            ...
            info!("PASS_debug entropy_state.device_id {} finished...", entropy_state.device_id);
            restore_helper(
                device.clone(),
                device,
